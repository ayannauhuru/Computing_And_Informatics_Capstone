...
function flushTable() {
    $db = Database::getDB();
    $query = 'DELETE FROM .stadiumReviews';
...
function write($data, $id) {
    $db = Database::getDB();
    $query = 'INSERT INTO .stadiumReviews (id, formatted_address, name, profile_photo_url, author_name, rating, text, relative_time_description) VALUES (:id, :formatted_address, :name, :profile_photo_url, :author_name, :rating, :text, :relative_time_description)';
...
function read() {
    $db = Database::getDB();
    $query = 'SELECT * FROM .stadiumReviews';
    $statement = $db->prepare($query);
    $statement->execute();
